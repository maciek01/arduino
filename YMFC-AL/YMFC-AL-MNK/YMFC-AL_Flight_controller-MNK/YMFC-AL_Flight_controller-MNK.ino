...
float pid_p_gain_yaw = 5.0;                //Gain setting for the pitch P-controller. //4.0
...
void setup(){
  ...
  /* if */ while  (eeprom_data[31] == 2 || eeprom_data[31] == 3)delay(10);
...
void loop(){
  ...
  if (start == 0) {
    ...
    //manage auto level setting
    auto_level = (PIND & B00001000) != 8;

    //blink if in acro and battery is not low
    if (!auto_level && !(battery_voltage < MIN_BATTERY_V && battery_voltage > 600)) {
  ...
  if (start == 2){                                                          //The motors are started.
    ...
    esc_1 = 950;                                                           //If start is not 2 keep a 1000us pulse for ess-1.
    esc_2 = 950;                                                           //If start is not 2 keep a 1000us pulse for ess-2.
    esc_3 = 950;                                                           //If start is not 2 keep a 1000us pulse for ess-3.
    esc_4 = 950;                                                           //If start is not 2 keep a 1000us pulse for ess-4.
